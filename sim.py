...
class Simulator():
	...
	def jump(self):
		...
		adjust = 1.32
		back_flip = np.array((0, -1 * adjust * np.pi / ( 4.0 ) , -1 * adjust * np.pi / ( 2.0 )  ,  0 , adjust * np.pi / ( 4.0 ) ,adjust * np.pi / ( 2.0 ) ,
 		0 , -1 * np.pi / ( 4.0 )  , -1 * np.pi / ( 2.0 )  , 0  , np.pi / ( 4.0 ) , np.pi / ( 2.0 )   ))
...
if __name__ == '__main__':
	...
	s = Simulator(path , start_pos ,start_orientation )			# initialize Simulator object
	s.go_to_start()
	s.pass_time(1000)
