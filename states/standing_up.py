...
class Standing_Up(State):
    ...
    def get_points(self, robot):
        ...
        dist = robot.trajectory_executor.current_position[0][2] - \
               robot.trajectory_executor.stand_position[0][2]
    ...
    def enter(self, robot):
        ...
        for leg in robot.trajectory_executor.default_pose:
            traj = robot.trajectory_executor.leg_trajectory_generator.compute_leg_linear_trajectory2(
                robot.trajectory_executor.current_position[l],
                robot.trajectory_executor.stand_position[l], num_points + 1)
            robot.trajectory_executor.leg_single_trajectory.append(
                list(zip(traj[0], traj[1], traj[2])))
            l += 1
        ...
        robot.trajectory_executor.modes = ["leg_single_trajectory",
                                           ...
                                           "leg_single_trajectory",
                                           "leg_single_trajectory"]
        robot.trajectory_executor.clock_max = len(
            robot.trajectory_executor.leg_single_trajectory[0]) - 1
        self.clock_max = len(
            robot.trajectory_executor.leg_single_trajectory[0]) - 1
